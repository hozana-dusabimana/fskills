...
let testLevelInfo;
let testQuick;
const RP31KBuild = 1; // fixes for legacy code made during RP31K
...
const startCheckpointTime = 90;
const extraCheckpointTime = 45;
...
let freeRide;
let score, highScore;
let lastScoreDistance = 0; // Track last distance for 1m score
let collisionScorePenalty = false; // Flag to reduce score on collision
window.showCheckPoint = false; // Show checkpoint only after first level
...
function gameStart() {
    ...
    // --- Score/HighScore ---
    score = 0;
    highScore = parseInt(localStorage[saveName + '_highScore'] || '0', 10);
    lastScoreDistance = Math.floor(playerVehicle.pos.z); // Always reset lastScoreDistance
    // --- End Score/HighScore ---
...
function gameUpdateInternal() {
    if (titleScreenMode) {
        ...
        if (!startCountdown && !freeRide && !gameOverTimer.isSet()) {
            ...
            // --- Score: score = distance (in km) * 0.5 ---
            distanceKm = playerVehicle.pos.z / 1000;
            score = Math.floor(distanceKm * 0.5);
            // --- End Score: score = distance (in km) * 0.5 ---
            // --- High Score Update ---
            if (score > highScore) {
                highScore = score;
                saveHighScore();
            ...
            // --- Show Checkpoint on next levels ---
            if (playerLevel > 0) showCheckPoint = true;
            // --- End Show Checkpoint ---
        }
    }
    updateCars();
    ...
    // --- Score: Overtake ---
    // Award points for each AI vehicle destroyed behind the player
    for (const v of vehicles) {
        if (v.destroyed && v.pos.z < playerVehicle.pos.z && !v._scoreGiven) {
            score += 10;
            v._scoreGiven = true;
        }
    }
    ...
    // --- Collision Score Penalty ---
    if (collisionScorePenalty) {
        score = Math.max(0, score - 3);
        collisionScorePenalty = false;
    }
    // --- End Collision Score Penalty ---
}
...
function saveHighScore() {
    localStorage[saveName + '_highScore'] = highScore;
}