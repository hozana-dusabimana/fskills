...
function generateTetures() {
    ...
    }
    {
        ...
        drawDoubleLineSign('GAME', 'Skills Drive', BLACK, 0, .42, .2);
        ...
        rectOutline(.5, .5, .9, .7, .04);
        // Draw 'RP' in blue, 25% smaller
        context.font = 'bold 0.825px Arial';
        context.textAlign = 'center';
        context.textBaseline = 'middle';
        context.fillStyle = 'blue';
        context.fillText('RP', .5, .55, .8);
        setupContext(7, 2);
        drawNgomaSign();
        ...
        // obstacles - realistic, cylindrical, even shorter cone
        setupContext(5, 5); // Cone
        // Shadow (grounded)
        color('rgba(0,0,0,0.22)');
        ellipse(.5, .99, .09, .025);
        // Base (gray, elliptical, wider than body)
        color('gray');
        ellipse(.5, .965, .06, .018);
        // Cone body (orange, trapezoid for cylindrical look, even shorter)
        color('orange');
        context.beginPath();
        context.moveTo(.48, .965); // left base
        context.lineTo(.52, .965); // right base
        context.lineTo(.515, .68); // right top (lowered more)
        context.lineTo(.485, .68); // left top (lowered more)
        context.closePath();
        context.fill();
        // White stripes
        color('white');
        rect(.5, .91, .018, .018);
        rect(.5, .85, .012, .012);
        // Highlight (vertical, subtle)
        color('rgba(255,255,255,0.13)');
        rect(.495, .85, .008, .08);

        setupContext(6, 5); // Pothole
        ...
        setupContext(7, 5); // Roadblock
        color('rgba(0,0,0,0.25)');
        ellipse(.5, .99, .19, .05); // wider shadow
        color('yellow');
        rect(.5, .89, .22, .18); // wider block
        color('rgba(255,255,180,0.8)');
        rect(.5, .80, .22, .05); // wider top
        color('rgba(180,180,0,0.7)');
        rect(.61, .89, .05, .18); // wider side
        color('black');
        for (let i = 0; i < 5; ++i)
            rect(.39 + i * .045, .89, .025, .18); // more stripes, spread wider
    }
    if (hardAlpha) {
        // make hard alpha
        const minAlpha = 99;
    ...
    function rectOutline(x = .5, y = .5, w = 1, h = 1, l = .05) { context.lineWidth = l; context.strokeRect(x - w / 2, y - h / 2, w, h); }
    ...
    function drawkarongiSign() {
        drawSignBackground();
        color(WHITE, 1);
        // text('RP Cyitabi',.5,.27,.5,.35,.02,'courier');
        text('RP karongi', .5, .66, .5, .9, .02, 'courier');
        color(hsl(1, .8, .5), 1);
        text('RP Cyitabi', .60, .27, .5, .5, .02, 'courier');
    }
    ...
    function drawNgomaSign() {
        ...
        text('RP Ngoma', .5, .3, .13, 1, .003);


        text('RP Gishari', .5, .5, .13, 1, .003);
    ...
    function drawMusanzeSign() {
        drawSignBackground(1, .4, WHITE, BLACK);
        color(BLACK, 1);
        text('RP-Musanze', .5, .22, .3, .9, .01);
    }
    ...
    function drawRPSign() {
        ...
        // Draw 'RP' in blue in the center of the sign
        context.font = 'bold 1.1px Arial';
        context.textAlign = 'center';
        context.textBaseline = 'middle';
        context.fillStyle = 'blue';
        context.fillText('RP', .5, .55, .8);
    ...
    function drawCheckpointSign(side) {
        ...
        rect(.5, 0, 1, .5);
        color(hsl(.3, .7, .5));
        text('CHECKPOINT new Stage', .5, .14, .16, .95, .02, undefined, undefined, 600);
    }
    ...
    function drawLicensePlate() {
        ...
        rect();
        color(hsl(.7, .9, .25), 1);
        text('RP-31K', .5, .6, 1, .9, .03, 'monospace');
    }
}