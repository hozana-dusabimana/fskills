...
function buildTrack()
{
    ...
    for(let i=0; i < trackEnd + 5e4; ++i)
    {
        ...
        if (track[i].level === 2) { // Level 2 (stage 3)
            // Random lane positions: -1 (left), 0 (center), 1 (right)
            const laneOffsets = [-0.7, 0, 0.7];
            if (i % 150 === 0) {
                const lane = laneOffsets[random.int(3)];
                track[i].addSprite(spriteList.cone, lane * (track[i].width / 2) * 0.7);
            }
            if (i % 200 === 50) {
                const lane = laneOffsets[random.int(3)];
                track[i].addSprite(spriteList.pothole, lane * (track[i].width / 2) * 0.7);
            }
            if (i % 300 === 100) {
                const lane = laneOffsets[random.int(3)];
                track[i].addSprite(spriteList.roadblock, lane * (track[i].width / 2) * 0.7);
            }
        }
    }
