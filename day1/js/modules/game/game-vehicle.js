...
function drawCars()
{
    // Only draw player vehicle
    playerVehicle.draw();
}

function updateCars()
{
    // Only update player vehicle
    playerVehicle.update();
}

// Remove spawnVehicle function since we don't need AI vehicles
...
class Vehicle
{
    constructor(z, color)
    {
        ...
        if (!this.color)
        {
            this.color = hsl(0,0,.1); // Default color for player vehicle
        }
    }

    update()
    {
        // Only player vehicle exists, no need for AI logic
        return;
    }
    ...
    draw()
    {
        ...
        const mcar = m1.multiply(buildMatrix(0, 0, vec3(450,500,450)));

        {
            // shadow
            glSetDepthTest(0,0); // no depth test for player shadow
            glPolygonOffset(60);
            ...
            const shadowSize = vec3(-720,200,600);
