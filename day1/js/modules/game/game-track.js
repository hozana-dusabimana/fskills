// Defines and manages the race track data and logic
'use strict';
...
function drawTrackScenery()
{
    ...
    if (!RP31KBuild) // final thing rendered, so no need to reset
    {
        glSetDepthTest();
        glEnableLighting = 1;
    }
...
class TrackSegment
{
    constructor(segmentIndex,offset,width)
    {
        ...
        // Add track side objects (telephone poles, etc)
        if (levelInfo.trackSideSprite && !this.sideStreet)
        {
            if (random.bool(levelInfo.trackSideChance))
            {
                const side = levelInfo.trackSideForce || random.sign();
                const offset = side * (width + random.float(1e3, 2e3));
                this.addSprite(levelInfo.trackSideSprite, offset);
            }
        }

        // Add billboards
        if (!this.sideStreet && random.bool(levelInfo.billboardChance))
        {
            if (segmentIndex % levelInfo.billboardRate === 0)
            {
                const billboardSprite = random.fromList(spriteList.billboards);
                const side = random.sign();
                const offset = side * (width + random.float(2e3, 4e3));
                this.addSprite(billboardSprite, offset, 0, levelInfo.billboardScale);
            }
        }
        ...
        else if (segmentIndex%checkpointTrackSegments == 0)
        {
            // checkpoint
            if (segmentIndex < levelGoal*checkpointTrackSegments) {
                // Only add checkpoint sign for stages 2 and 3
                const stage = segmentIndex / checkpointTrackSegments;
        ...
        if (segmentIndex == 30)
        {
            ...
            this.addSprite(spriteList.sign_RP31K,or);
            if (RP31KBuild)
                random.seed = 1055752394; // hack, reset seed for RP31K
        }
    }
